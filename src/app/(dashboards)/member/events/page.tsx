        ...
        events: [
          {
            ...
            slug: "BISF",
            description: "Join us for an evening of captivating stories...",
            date: "2023-12-15",
...
export default function MemberEventsPage(){
  ...
  const getMyRole = (meeting: Meeting) => {
    if (!data) return "None"
                ...
                <Card key={meeting.id}>
                  <CardHeader>
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      ...
                      <Badge className="self-start md:self-center bg-blue-100 text-blue-800">Your Role: {getMyRole(meeting)}</Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                          ...
                          {meeting.roles?.map((role, index) => (
                            <div
                              ...
                              className={`flex justify-between p-2 rounded-md ${
                                role.assignedTo === data?.currentUserId ? "bg-blue-50 border border-blue-200" : "bg-gray-50"
                              }`}
                            >
                              <span className="font-medium text-gray-700">{role.role}</span>
                              <span className="text-gray-500">
                                {role.assignedTo === data?.currentUserId ? "You" : role.assignedTo ? "Assigned" : "Unassigned"}
                              </span>
                            </div>
                          ))}
                        </div>
