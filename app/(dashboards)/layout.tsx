...
export default async function DashboardLayout({
  ...
  return (
    <div lang="en" className="min-h-screen bg-white text-slate-900">
      <div className="flex">
        <aside className="w-64 fixed left-0 top-0 h-screen overflow-auto z-40 bg-gray-100 text-black p-4 shadow-lg">
          {/* Brand */}
          <div className="mb-6">
            ...
            <ul className="space-y-1">
              {navItems && navItems.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="flex items-center gap-3 px-3 py-2 rounded text-black/80 hover:bg-white/5"
                  >
        ...
        <main className="flex-1 ml-64 w-full">
          ...
          <footer className="p-3 border-t border-slate-100 bg-white text-center text-sm text-slate-500">
